/**
 * System prompts and helper functions for AI categorization
 */
...
export const SYSTEM_PROMPT = `You are an expert analyst categorizing Twitter/X users based on their x402-related activity.
...
export const ENHANCED_SYSTEM_PROMPT = `You are an expert analyst finding genuine crypto/web3 KEY OPINION LEADERS (KOLs) - independent voices who provide valuable insights, NOT company promoters.
...
export const SECONDARY_SYSTEM_PROMPT = `You are an expert analyst classifying previously-uncategorized Twitter/X accounts for the x402 ecosystem.
...
export const BATCH_SYSTEM_PROMPT = `You are an expert analyst finding genuine crypto/web3 KEY OPINION LEADERS (KOLs) - independent voices who provide valuable insights, NOT company promoters.
...
export const BATCH_SECONDARY_SYSTEM_PROMPT = `You are an expert analyst classifying previously-uncategorized Twitter/X accounts for the x402 ecosystem.
...
/**
 * Format tweets for display with full engagement metrics
 */
export function formatTweets(tweets: RapidApiTweet[]): string {
  return tweets
    .map((tweet, index) => {
 ...
 */
export function formatTweetsEnhanced(
  tweets: RapidApiTweet[] | null | undefined,
  maxCharsPerTweet: number = 200
): string {
  ...
  return tweets
    .map((tweet, index) => {
      const date = new Date(tweet.created_at).toLocaleDateString();
      const text =
        (tweet.text || '').length > maxCharsPerTweet
 ...
 */
export function calculateEngagementStats(tweets: RapidApiTweet[]): {
  totalViews: number;
  totalLikes: number;
...
/**
 * Build user prompt for basic categorization
 */
export function buildUserPrompt(account: Account, tweets: RapidApiTweet[]): string {
 ...
 */
export function buildSecondaryUserPrompt(
  account: Account,
  x402Tweets: RapidApiTweet[],
  generalTweets: RapidApiTweet[]
): string {
  ...
  return `Classify this Twitter user as DEVELOPER, ACTIVE_USER, or UNCATEGORIZED:
...
---
Based on the evidence above, determine if this is a DEVELOPER, ACTIVE_USER, or UNCATEGORIZED.`;
}

/**
 * Build batch user prompt for multiple users
 */
export function buildBatchUserPrompt(inputs: BatchCategorizationInput[]): string {
 ...
 */
export function buildBatchSecondaryUserPrompt(inputs: BatchCategorizationInput[]): string {
  const usersData = inputs.map((input, index) => {
    const { account, x402Tweets, generalTweets } = input;
    const x402Formatted = formatTweetsEnhanced(x402Tweets);
    const generalFormatted = formatTweetsEnhanced(generalTweets);
...
/**
 * Parse basic AI response
 */
export function parseAIResponse(content: string): AICategoryResult {
  try {
 ...
 */
export function parseEnhancedAIResponse(content: string): EnhancedAICategoryResult {
  try {
    const jsonMatch = content.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      throw new Error('No JSON found in response');
 ...
 */
export function parseSecondaryAIResponse(content: string): AICategoryResult {
  try {
    const jsonMatch = content.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      throw new Error('No JSON found in response');
...
/**
 * Parse batch AI response for enhanced categorization
 */
export function parseBatchAIResponse(content: string): Map<string, EnhancedAICategoryResult> {
  ...
  try {
    ...
    for (const item of parsed) {
      ...
      const validCategories = ['KOL', 'UNCATEGORIZED'];
      const category = validCategories.includes(item.category) ? item.category : 'UNCATEGORIZED';
 ...
 */
export function parseBatchSecondaryAIResponse(content: string): Map<string, AICategoryResult> {
